...
int main(int argc, char* argv[])
{
	...
	// 创建服务端套接字
	sServer = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP);//SOCK_DGRAM意味着这是一个UDP(数据报socket)
	...
	while (true)
	{
		...
		/* 服务器作为接收方,使用recvfrom()
		 * 参数代表 socket id,字符类型数组,接收字符数,接收属性的比特集合,指向远端socket地址指针,地址长度
		 * 返回值 -1代表遇到错误 或者 收到的字符数
		 * */
