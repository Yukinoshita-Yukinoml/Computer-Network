...
namespace Code{//命名空间Code

	//constexpr int BytesPerFrame = 3738;
	constexpr int BytesPerFrame = 1242;//字节帧 1242
	constexpr int FrameSize = 108;//帧大小(不是实际分辨率)
	constexpr int FrameOutputRate = 10;//
	constexpr int SafeAreaWidth = 2;//安全区大小
	...
	Mat ScaleToDisSize(const Mat& src){//把src放大十倍
		Mat dis;
	...
	uint16_t CalCheckCode(const unsigned char* info, int len, bool isStart, bool isEnd, uint16_t frameBase){//unsigned short == uint16_t
		uint16_t ans = 0;
		int cutlen = (len / 2)*2;
	...
	void BulidInfoRect(Mat& mat, const char* info, int len,int areaID){
		const unsigned char* pos = (const unsigned char*)info;//输入位置
		const unsigned char* end = pos + len;//结束为开始位置+长度
		for (int i = 0; i < areapos[areaID][0][0]; ++i){//
			uint32_t outputCode = 0;//初始化输出码为0
			for (int j = 0; j < areapos[areaID][0][1]/8; ++j){
				outputCode |= *pos++;//按位或等于
				/*for (int k = 0; k < 3; ++k)
				{
					outputCode <<= 8;
					if (pos != end)
	...
	void Main(const char* info, int len,const char * savePath,const char * outputFormat,int FrameCountLimit){
		...
		if (len <= 0);
			...
			for (int i = len; i <= BytesPerFrame; ++i)
				BUF[i] = rand() % 256;//BUF[i] 赋值得 随机数用于RGB // for
			...
			while (len > 0 && FrameCountLimit > 0){
				...
				if (len - BytesPerFrame > 0){
					if (FrameCountLimit>0)
						output = ScaleToDisSize(CodeFrame(FrameType::Normal, info, BytesPerFrame, ++i));//if
					else output = ScaleToDisSize(CodeFrame(FrameType::End, info, BytesPerFrame, ++i));//else
				}//if
				else{
					unsigned char BUF[BytesPerFrame + 5];
					memcpy(BUF, info, sizeof(unsigned char) * len);
